...
pub fn handle(
    ...
    let result: Result<serde_json::Value, String> = match tool_name {
        // ── 查询类 ──
        "search_entities" => {
            dispatch_search_entities(&arguments, &app)
        }
        "get_entity_context" => {
            dispatch_get_entity_context(&arguments, &app)
        }
        ...
        other => Err(format!("Unknown tool: '{}'. See GET /tools for available tools.", other)),
    };
...
fn dispatch_search_entities(
    ...
    for node in &all {
        // code 精确匹配优先
        if let Some(c) = code {
            if node.property("code").and_then(|v| v.as_str()) != Some(c) {
                continue;
            }
        }
        if let Some(t) = typ {
            if node.property("type").and_then(|v| v.as_str()) != Some(t) {
                continue;
            }
        }
        if let Some(s) = side {
            if node.property("command_side").and_then(|v| v.as_i64()) != Some(s) {
                continue;
            }
        }
        if let Some(k) = kw {
            let name = node.property("name").and_then(|v| v.as_str()).unwrap_or("");
            let desc = node.property("description").and_then(|v| v.as_str()).unwrap_or("");
            if !name.contains(k) && !desc.contains(k) {
                continue;
            }
        }

        let props: serde_json::Map<_, _> = node.properties.iter()
                ...
                if !visited.contains(&neighbor_code) {
                    ...
                    for label in &["Entity", "Type", "Patrol", "Behavior"] {
                        ...
                        if let Some(n) = nodes.iter().find(|n| {
                            ...
                            context["neighbors"].as_array_mut().unwrap().push(
                                serde_json::json!({
                                    "code": neighbor_code,
                                    "label": label,
                                    "relation": r.rel_type,
...
fn dispatch_load_swrl_rule(
    ...
    let rule_text = args.get("rule_text").and_then(|v| v.as_str())
        .ok_or("'rule_text' is required")?;

    app.reasoner.load_swrl_rule(rule_text)
...
fn dispatch_execute_reasoning(
    ...
    let _incremental = args.get("incremental").and_then(|v| v.as_bool()).unwrap_or(false);

    let report = app.reasoner.reason().map_err(|e| format!("Reasoning error: {}", e))?;
...
fn dispatch_simulate_patrol(
    ...
    let wps_raw = args.get("waypoints").cloned().unwrap_or(serde_json::Value::Array(vec![]));
    ...
    // 匹配实体
    let entity_code = match find_entity_by_code(app, args.get("attacker").and_then(|v| v.as_str()), &waypoints) {
        (code, lat, lon, alt, speed) => {
...
fn dispatch_simulate_strike(
    ...
    let code = args.get("code").and_then(|v| v.as_str()).unwrap_or("");
    let name = args.get("name").and_then(|v| v.as_str()).unwrap_or("");
    let attacker_code = args.get("attacker").and_then(|v| v.as_str())
        .ok_or("'attacker' is required")?;
    let target_code = args.get("target").and_then(|v| v.as_str())
        .ok_or("'target' is required")?;
    let weapon_type = args.get("weapon_type").and_then(|v| v.as_str())
    ...
    let (a_lat, a_lon, a_alt, a_conf) = match &attacker {
        ...
        None => (
            args.get("attacker_lat").and_then(|v| v.as_f64()).unwrap_or(0.0),
            args.get("attacker_lon").and_then(|v| v.as_f64()).unwrap_or(0.0),
            args.get("attacker_alt").and_then(|v| v.as_f64()).unwrap_or(0.0),
            args.get("confidence").and_then(|v| v.as_f64()).unwrap_or(0.7),
        ),
    };
    ...
    let (t_lat, t_lon, t_depth) = match &target {
        ...
        None => (
            args.get("target_lat").and_then(|v| v.as_f64()).unwrap_or(0.0),
            args.get("target_lon").and_then(|v| v.as_f64()).unwrap_or(0.0),
            args.get("target_depth").and_then(|v| v.as_f64()).unwrap_or(0.0),
        ),
    };

    let weapon_range = args.get("weapon_range_m").and_then(|v| v.as_f64()).unwrap_or(10_000.0);
    let confidence = args.get("confidence").and_then(|v| v.as_f64()).unwrap_or(a_conf);
    ...
    let input = StrikeInput {
        ...
        attacker_lat: a_lat, attacker_lon: a_lon, attacker_alt: a_alt,
        target_code: target_code.to_string(),
        target_lat: t_lat, target_lon: t_lon, target_depth: t_depth,
...
fn dispatch_infer_forward(
    ...
    let source_code = result.source.property("code").and_then(|v| v.as_str()).unwrap_or(id);
    ...
    let avg_conf = if with_conf.is_empty() { None } else {
        Some(with_conf.iter().sum::<f64>() / with_conf.len() as f64)
    };

    let chain_json: Vec<serde_json::Value> = result.chain.iter().map(|hop| {
        let target_id: &str = &hop.target_id;
        let target_json = hop.target_node.as_ref()
            .map(|n| node_to_json(n))
            .unwrap_or_else(|| serde_json::json!({"id": target_id}));

        let next_rels: Vec<serde_json::Value> = hop.target_outgoing.iter().map(|rc| {
            serde_json::json!({
        ...
        let rules: Vec<serde_json::Value> = hop.matching_rules.iter().map(|rm| {
            serde_json::json!({
...
fn prop_to_json(v: &ontology_storage::mapper::graph::property::PropertyValue) -> serde_json::Value {
    ...
    match v {
        ...
        PropertyValue::List(items) => serde_json::Value::Array(items.iter().map(prop_to_json).collect()),
        PropertyValue::Map(m) => {
            let map: serde_json::Map<_, _> = m.iter().map(|(k,v)| (k.clone(), prop_to_json(v))).collect();
...
fn node_to_json(n: &ontology_storage::mapper::graph::node::Node) -> serde_json::Value {
    let props: serde_json::Map<_, _> = n.properties.iter()
        .map(|(k, v)| (k.clone(), prop_to_json(v)))
        .collect();
    serde_json::json!({
        "labels": n.labels,
...
fn prop_as_f64(val: Option<&ontology_storage::mapper::graph::property::PropertyValue>) -> Option<f64> {
    use ontology_storage::mapper::graph::property::PropertyValue;
    match val {
...
fn find_entity_by_label(app: &AppState, code: &str) -> Option<ontology_storage::mapper::graph::node::Node> {
    if code.is_empty() { return None; }
    let all = app.repo.get_nodes_by_label("Entity").unwrap_or_default();
    all.into_iter().find(|n| n.property("code").and_then(|v| v.as_str()) == Some(code))
}
...
fn find_entity_by_code(
    ...
    for n in &all {
        if let Some(speed) = prop_as_f64(n.property("speed")) {
            let code = n.property("code").and_then(|v| v.as_str()).unwrap_or("?").to_string();
