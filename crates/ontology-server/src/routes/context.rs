...
pub fn handle(
    ...
    let all_entities = app.repo.get_nodes_by_label("Entity").unwrap_or_default();
    let entity = match all_entities.iter().find(|n| n.property("code").and_then(|v| v.as_str()) == Some(code)) {
        Some(n) => n.clone(),
    ...
    for rel in &outgoing_raw {
        ...
        let tprops: serde_json::Map<_, _> = target.iter()
    ...
    for n in &all {
        ...
        if seen.contains(ncode) { continue; }
    ...
    // ── 分类层级 ──
    let etype = entity.property("type").and_then(|v| v.as_str()).unwrap_or("");
...
fn p2j(v: &ontology_storage::PropertyValue) -> serde_json::Value {
    match v {
        ...
        ontology_storage::PropertyValue::List(v) => serde_json::Value::Array(v.iter().map(p2j).collect()),
...
fn get_type_ancestors(app: &crate::app::AppState, type_name: &str) -> Vec<String> {
    ...
    if type_name.is_empty() { return path; }
    // 简化 BFS: 查 Type 节点沿 subClassOf 向上
    let types = app.repo.get_nodes_by_label("Type").unwrap_or_default();
