...
pub fn handle_get(state: &Arc<Mutex<AppState>>) -> (u16, String) {
    ...
    let rules: Vec<serde_json::Value> = app.reasoner.rules().iter().map(|r| {
        serde_json::json!({
    ...
    (200, serde_json::json!({ "count": rules.len(), "rules": rules }).to_string())
}
...
pub fn handle_post(
    ...
    for node in &rule_nodes {
        ...
        let enabled = node.property("enabled")
            .and_then(|v| v.as_bool()).unwrap_or(true);

        if !enabled || text.is_empty() { continue; }
