...
pub fn handle(
    ...
    for node in &all {
        let match_type = q.get("type").and_then(|v| v.as_str())
            .map(|t| node.property("type").and_then(|p| p.as_str()) == Some(t))
            .unwrap_or(true);

        let match_side = q.get("command_side").and_then(|v| v.as_i64())
            .map(|s| node.property("command_side").and_then(|p| p.as_i64()) == Some(s))
            .unwrap_or(true);

        let match_kw = q.get("keyword").and_then(|v| v.as_str())
            .map(|kw| {
                let name = node.property("name").and_then(|p| p.as_str()).unwrap_or("");
                let desc = node.property("description").and_then(|p| p.as_str()).unwrap_or("");
        ...
        let match_spatial = if let (Some(lat), Some(lon), Some(r)) = (
            ...
            if let Some(sp) = node.property("Space_abs").and_then(|p| p.as_list()) {
                let nlat = sp.get(0).and_then(|v| {
                    match v { ontology_storage::PropertyValue::Float(f) => Some(*f), _ => v.as_i64().map(|i| i as f64) }
                }).unwrap_or(0.0);
                let nlon = sp.get(1).and_then(|v| {
                    match v { ontology_storage::PropertyValue::Float(f) => Some(*f), _ => v.as_i64().map(|i| i as f64) }
                }).unwrap_or(0.0);
...
fn prop_to_json(v: &ontology_storage::PropertyValue) -> serde_json::Value {
    match v {
        ...
        ontology_storage::PropertyValue::Float(f) => {
            serde_json::Number::from_f64(*f).map(serde_json::Value::Number).unwrap_or(serde_json::Value::Null)
        }
        ontology_storage::PropertyValue::Boolean(b) => serde_json::Value::Bool(*b),
        ontology_storage::PropertyValue::List(v) => serde_json::Value::Array(v.iter().map(prop_to_json).collect()),
