...
本 crate 的规则、查询、约束使用统一前缀区分不同语言，前缀后紧跟语言表达式：

| 前缀 | 语言 | 说明 |
|------|------|------|
| `owl2:` | OWL2-DL 语言 | 描述逻辑（Description Logic），用于 DWL2 查询模块的类表达式 |
| `swrl:` | SWRL 语言 | 语义 Web 规则语言（Semantic Web Rule Language），用于规则推理和行为引擎 |
| `sh:` | SHACL 语言 | 形状约束语言（Shapes Constraint Language），用于图节点验证 |
...
| **REASONER-010** | 语言前缀路由在 `reasoner.rs` 中完成：`owl2:` → DWL2、`swrl:` → SWRL、`sh:` → SHACL。前缀解析由 `language.rs` 模块提供，引擎本身不感知前缀 |
...
| **CONF-002** | 熔断阈值默认 0.30（Exercise 模式），WarFighting 模式降至 0.15 |
...
├── language.rs            # ✅ 语言前缀解析 (owl2:/swrl:/sh:)
├── logger.rs              #   日志初始化
├── dwl2/
│   ├── mod.rs             #   模块声明
...
│   ├── fuse.rs            #   置信度熔断器
│   ├── policy.rs          #   策略引擎 (OperationMode + SourceCategory)
│   └── README.md          #   模块文档
├── spatial/
│   ├── mod.rs             #   模块声明
