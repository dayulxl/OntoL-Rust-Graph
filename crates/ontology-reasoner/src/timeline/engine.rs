...
impl TimelineEngine {
    ...
    pub fn simulate(&self, input: &TimelineInput) -> TimelineResult {
        ...
        log_out!(
            "   📐 总航程: {:.2} km  总耗时: {}s ({:.1} min)",
            total_km,
            total_dur,
            total_dur as f64 / 60.0
        );
        ...
        let desc: Vec<String> = segments
            .iter()
            .map(|s| {
                format!(
                    "{}(第{}航点,{:.0}m,{:.1}s)",
                    s.action, s.seq, s.dist_m, s.time_s
                )
            })
            .collect();
        ...
        for (i, seg) in segments.iter().enumerate() {
            ...
            log_out!(
                "   [{}/{}] {} {:5} → lat={:.4} lon={:.4} alt={:.0}  航段{:.0}m {:.1}s  剩余{:.0}s",
                seg.seq,
                segments.len(),
                bar,
                seg.action,
        ...
        TimelineResult {
            patrol_code: patrol_code.clone(),
            patrol_name: patrol_name.clone(),
            entity_code: entity_code.clone(),
            segments,
            total_distance_m,
            total_distance_km: format!("{:.2}", total_km),
            total_time_s,
            duration: total_dur,
            precondition,
            effect,
...
impl TimelineEngine {
    ...
    pub fn simulate_strike(&self, input: &StrikeInput) -> StrikeResult {
        ...
        let hit_probability = (base_prob * depth_factor).clamp(0.0, 1.0);
        ...
        let total_time_s = if weapon_speed > 0.0 {
            distance_m / weapon_speed
        } else {
            0.0
        };
        log_out!(
        ...
        let cost = format!(
            ...
            distance_m,
            total_time_s,
            if hit_probability > 0.3 { 1u8 } else { 2u8 }
        );
...
mod tests {
    ...
    fn test_simulate() {
        ...
        let input = TimelineInput {
            patrol_code: "TEST".into(),
            patrol_name: "Test".into(),
            patrol_id: "u".into(),
            waypoints: vec![
                WaypointInput {
                    seq: 1,
